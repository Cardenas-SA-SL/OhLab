 ...
 * that writes $GROK_HOME/hooks/ohlab-status.json. Canvas control needed nothing new — grok scans
 * `~/.claude/skills`, where manage-ohlab-canvas already lives (asserted below). Context links did
...
describe('grok capabilities', () => {
  ...
  it('reports status through its own hooks', () => {
    ...
    // that writes $GROK_HOME/hooks/ohlab-status.json. Both exist now, so the badge, the unread
  ...
  it('drives the canvas, on the skill that is already installed for claude', () => {
    ...
    // manage-ohlab-canvas is already written. See config.control.test.ts.
