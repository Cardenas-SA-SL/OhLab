                  ...
                  <div key={project.projectId} className="space-y-2 rounded-lg border border-border p-3">
                    ...
                    {project.members?.map((member) => (
                      <div key={member.accountId} className="flex items-center justify-between gap-3 text-sm">
                        <span><span aria-label={member.online ? 'online' : 'offline'} className={`mr-2 inline-block h-2 w-2 rounded-full ${member.online ? 'bg-green-500' : 'bg-muted'}`} />{member.name} <span className="text-xs text-muted">{member.machineLabel ? `· ${member.machineLabel} ` : ''}· {member.role} · {member.status}</span></span>
