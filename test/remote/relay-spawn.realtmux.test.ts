...
import { localTmuxEnterArgs, localTmuxPasteArgs, pasteBufferName, sessionName } from '../../src/core/tmux-naming'
...
suite('REAL tmux: relay-bridged agent spawn', () => {
  it('runs the clean launch through the host paste-buffer API', async () => {
    ...
    await bridgedApi.pty.sendText('remote-agent', queued.data.pendingLaunch!.command)
